...
const simulateScheduling = (processes, algorithm, mode) => {
    ...
    while (completedProcesses.length < processes.length && safetyBreak < MAX_SIM_TIME) {
        ...
        if (currentCpuProcess) {
            ...
            // Merge Gantt entries if same PID and contiguous
            if (gantt.length > 0 && gantt[gantt.length - 1].pid === currentCpuProcess.pid && gantt[gantt.length - 1].end === execStart) {
...
export const submitSchedule = async (mode, humanSchedule) => {
    ...
    // Define mock cost weights for Real-Time

    const weights = { avg_wait_weight: 0.5, fairness_weight: 0.2, starvation_weight: 5.0, cs_penalty: 1.0, pre_cost: 0.5 };



    // --- Simulate Human Result ---

    const humanSimResult = simulateScheduling(humanSchedule, 'FCFS', mode); // Treat manual as FCFS on given order
    ...
    const humanResult = {
        ...
        completed_processes: humanSimResult.completed_processes,
    ...
    humanResult.metrics.final_score = (
        ...
        humanResult.completed_processes.filter(p => p.priority === 1 && p.waiting_time > 10).length * 1000
    ...
    const aiAlgo = 'Priority'; // AI prioritizes critical cases in Real-Time

    let aiScheduleOptimized = JSON.parse(JSON.stringify(humanSchedule));

    aiScheduleOptimized.sort((a,b) => a.priority - b.priority);
    ...
    aiResult.metrics.final_score = (
        ...
        aiResult.completed_processes.filter(p => p.priority === 1 && p.waiting_time > 10).length * 1000
    ...
    if (humanResult.metrics.final_score < aiResult.metrics.final_score) {
        ...
        explanation = "You beat the AI! Excellent work managing the medical workload efficiently.";
        ...
        if (humanResult.metrics.average_waiting_time > aiResult.metrics.average_waiting_time * 1.5) {
            ...
            humanProblems.push("your overall treatment time was inefficient");
...
export const getLeaderboard = async () => {
    ...
    return [
        ...
        { name: 'WOPR', score: 35, mode: 'Real-Time' },

        { name: 'Mother', score: 48, mode: 'Real-Time' },

        { name: 'Cortana', score: 55, mode: 'Real-Time' },

        { name: 'GLaDOS', score: 62, mode: 'Real-Time' },
